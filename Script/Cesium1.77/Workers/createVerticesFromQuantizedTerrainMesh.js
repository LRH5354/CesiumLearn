define([
  ...
  var ge = 32767,
    ...
    fe = new ae.Cartesian3(),
    Ee = new ae.Cartographic(),
  ...
  function xe(e, t, r, n, i, o, a, s, d) {
    ...
    for (var m = e.length, g = 0; g < m; ++g) {
      var T = e[g],
        p = r[T],
        T = n[T];
      (Ee.longitude = oe.CesiumMath.lerp(I, l, T.x)),
        (Ee.latitude = oe.CesiumMath.lerp(h, u, T.y)),
        (Ee.height = p - t);
      p = o.cartographicToCartesian(Ee, Te);
      se.Matrix4.multiplyByPoint(a, p, p),
        ae.Cartesian3.minimumByComponent(p, s, s),
        ae.Cartesian3.maximumByComponent(p, d, d),
        (c = Math.min(c, Ee.height));
    }
    return c;
  }
  function Me(e, t, r, n, i, o, a, s, d, c, u, h, l, I, m) {
    var g = ie.defined(a),
      ...
      f = d.east,
      E = d.west;
    f < E && (f += oe.CesiumMath.TWO_PI);
    for (var y = r.length, v = 0; v < y; ++v) {
      var N = r[v],
        w = i[N],
        x = o[N];
      (Ee.longitude = oe.CesiumMath.lerp(E, f, x.x) + I),
        (Ee.latitude = oe.CesiumMath.lerp(p, T, x.y) + m),
        (Ee.height = w - c);
      var M,
        C,
        b = s.cartographicToCartesian(Ee, Te);
      g &&
        ((M = 2 * N),
        (ye.x = a[M]),
        (ye.y = a[1 + M]),
        ...
        n.hasWebMercatorT &&
          (C =
            (ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
              ...
              h) *
            l),
        (t = n.encode(e, t, b, x, Ee.height, ye, C));
    }
  }
  function Ce(e, t) {
    var r;
  ...
  return a(function (e, t) {
    var r,
      ...
      f = se.Transforms.eastNorthUpToFixedFrame(p, I),
      E = se.Matrix4.inverseTransformation(f, new se.Matrix4());
    s &&
    ...
    for (
      ...
      (Ee.longitude = oe.CesiumMath.lerp(c, h, _)),
        (Ee.latitude = oe.CesiumMath.lerp(u, l, H)),
        (Ee.height = k),
        (P = Math.min(Ee.longitude, P)),
        (W = Math.max(Ee.longitude, W)),
        (D = Math.min(Ee.latitude, D)),
        (B = Math.max(Ee.latitude, B));
      V = I.cartographicToCartesian(Ee);
        ...
        s &&
          (b[F] =
            (ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
              ...
              r) *
            n),
        se.Matrix4.multiplyByPoint(E, V, Te),
    ...
    var O,
      G,
      Y,
      z = Ce(e.westIndices, function (e, t) {
        return x[e].y - x[t].y;
      }),
      R = Ce(e.eastIndices, function (e, t) {
        return x[t].y - x[e].y;
      }),
      L = Ce(e.southIndices, function (e, t) {
        return x[t].x - x[e].x;
      }),
      U = Ce(e.northIndices, function (e, t) {
        return x[e].x - x[t].x;
      });
    1 !== m &&
      ((G = se.BoundingSphere.fromPoints(C)),
      (O = le.OrientedBoundingBox.fromRectangle(d, g, T, I))),
      (1 !== m || g < 0) &&
        (Y = new Ie.EllipsoidalOccluder(
          I
        ).computeHorizonCullingPointPossiblyUnderEllipsoid(p, C, g));
    var j = g,
      j = Math.min(
        j,
        xe(e.westIndices, e.westSkirtHeight, M, x, d, I, E, S, A)
      );
    (j = Math.min(
      j,
      xe(e.southIndices, e.southSkirtHeight, M, x, d, I, E, S, A)
    )),
      (j = Math.min(
        j,
        xe(e.eastIndices, e.eastSkirtHeight, M, x, d, I, E, S, A)
      )),
      (j = Math.min(
        j,
        xe(e.northIndices, e.northSkirtHeight, M, x, d, I, E, S, A)
      ));
    for (
      var q,
        Q,
        ...
        Z = new Ie.TerrainEncoding(X, j, T, f, w, s),
        J = Z.getStride(),
      ...
      w &&
        ...
        1 !== m &&
          ...
          (Q = se.Transforms.eastNorthUpToFixedFrame(C[te], I, we)),
        ...
        (ee = Z.encode($, ee, C[te], x[te], M[te], ye, b[te]));
