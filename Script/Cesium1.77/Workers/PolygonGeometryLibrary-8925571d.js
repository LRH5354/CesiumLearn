define([
  ...
  (R.subdivideRhumbLineCount = function (e, t, r, i) {
    ...
    (R.subdivideLine = function (e, t, r, i) {
      ...
      for (var u, l, h, c = 0, f = 0; f < n; f++) {
        var p =
          ...
          E.Cartesian3.multiplyByScalar(y, l / h, y),
          E.Cartesian3.add(p, y, y),
          [y.x, y.y, y.z]);
        (s[c++] = p[0]), (s[c++] = p[1]), (s[c++] = p[2]);
      }
      return s;
    }),
    (R.subdivideRhumbLine = function (e, t, r, i, n) {
      ...
      for (var l = 0, h = 0; h < o; h++) {
        var c = a.interpolateUsingSurfaceDistance(h * s, m),
          c = e.cartographicToCartesian(c, C);
        (u[l++] = c.x), (u[l++] = c.y), (u[l++] = c.z);
      }
      return u;
    });
    ...
    (R.polygonOutlinesFromHierarchy = function (e, t, r) {
      ...
      for (o.enqueue(e); 0 !== o.length; ) {
        var s = o.dequeue(),
          u = s.positions;
        if (t)
          for (n = u.length, h = 0; h < n; h++)
            r.scaleToGeodeticSurface(u[h], u[h]);
        if (
          !(
          ...
          for (var l = s.holes ? s.holes.length : 0, h = 0; h < l; h++) {
            var c = s.holes[h],
              f = c.positions;
            if (t)
            ...
            if (
              ...
              var p = 0;
              for (
                I.defined(c.holes) && (p = c.holes.length), i = 0;
                i < p;
                i++
              )
                o.enqueue(c.holes[i]);
    ...
    (R.polygonsFromHierarchy = function (e, t, r, i) {
      ...
      for (o.enqueue(e); 0 !== o.length; ) {
        ...
        if (
          ...
          if (I.defined(c)) {
            var f = [],
              p = D.PolygonPipeline.computeWindingOrder2D(c);
            p === D.WindingOrder.CLOCKWISE &&
              (c.reverse(), (l = l.slice().reverse()));
            for (
              var d,
                y = l.slice(),
                g = I.defined(h) ? h.length : 0,
                v = [],
                m = 0;
              m < g;
              m++
            ...
            n.push({ outerRing: l, holes: v }),
              a.push({ positions: y, positions2D: c, holes: f });
          }
        }
      }
  ...
  (R.computeBoundingRectangle = function (e, t, r, i, n) {
    for (
      ...
      var f = E.Cartesian3.clone(r[c], q);
  ...
  (R.computeWallGeometry = function (e, t, r, i, n) {
    ...
    if (i)
      ...
      var h = x.CesiumMath.chordLength(r, t.maximumRadius),
        c = 0;
      if (n === M.ArcType.GEODESIC)
        for (s = 0; s < u; s++)
          c += R.subdivideLineCount(e[s], e[(s + 1) % u], h);
      else if (n === M.ArcType.RHUMB)
        for (s = 0; s < u; s++)
          c += R.subdivideRhumbLineCount(t, e[s], e[(s + 1) % u], h);
      for (o = 3 * (c + u), a = new Array(2 * o), s = 0; s < u; s++) {
        var f,
          p = e[s],
          d = e[(s + 1) % u];
        n === M.ArcType.GEODESIC
          ? (f = R.subdivideLine(p, d, h, k))
          : n === M.ArcType.RHUMB && (f = R.subdivideRhumbLine(t, p, d, h, k));
