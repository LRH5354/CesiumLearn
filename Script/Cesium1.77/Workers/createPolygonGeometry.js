define([
  ...
  function L(e) {
    ...
    if (r.st || r.normal || r.tangent || r.bitangent || a) {
      ...
      G =
        0 !== m
          ? ((b = U.Quaternion.fromAxisAngle(c._plane.normal, m, ue)),
      ...
      for (var F = 0; F < n; F += 3) {
        ...
        r.st &&
          ...
          (L = c.projectPointOntoPlane(D, se)),
          Y.Cartesian2.subtract(L, d, L),
          (N = W.CesiumMath.clamp(L.x / p.width, 0, 1)),
          (H = W.CesiumMath.clamp(L.y / p.height, 0, 1)),
          u && ((f[C + V] = N), (f[C + 1 + V] = H)),
          l && ((f[C] = N), (f[C + 1] = H)),
          (C += 2)),
  ...
  function b(e, t, r, o, a) {
    ...
    if (r === c.ArcType.RHUMB) return Y.Rectangle.fromCartesianArray(e, t, a);
  ...
  function v(e, t, r, o) {
    for (
      ...
      p = 0 <= p ? p : p + W.CesiumMath.TWO_PI;
      (o.westOverIDL = Math.min(o.westOverIDL, p)),
        (o.eastOverIDL = Math.max(o.eastOverIDL, p));
    }
  }
  var N = [];
  function P(e) {
    var t,
      ...
      p = z.defaultValue(e.extrudedHeight, u);
    l || ((t = Math.max(u, p)), (p = Math.min(u, p)), (u = t)),
      ...
      (this._arcType = z.defaultValue(e.arcType, c.ArcType.GEODESIC)),
  ...
  return (
    ...
    (P.computeRectangle = function (e, t) {
      var r = z.defaultValue(e.granularity, W.CesiumMath.RADIANS_PER_DEGREE),
        o = z.defaultValue(e.arcType, c.ArcType.GEODESIC),
    ...
    (P.createGeometry = function (e) {
      var t = e._vertexFormat,
        ...
        p = i.positions;
      if (!(p.length < 3)) {
        var c = O.EllipsoidTangentPlane.fromPoints(p, r),
        ...
        if (0 !== y.length) {
          p = y[0].outerRing;
          var g,
            p = F.PolygonGeometryLibrary.computeBoundingRectangle(
              c.plane.normal,
            ...
            for (
              ...
              var _,
                v = (function (e, t, r, o, a, i, n, s, l) {
                  var u = { walls: [] };
                  if (i || n) {
                    var p = F.PolygonGeometryLibrary.createGeometryFromPositions(
                        e,
                        t,
                        r,
                        a,
                      ...
                      t = p.attributes.position.values,
                      c = p.indices;
                    if (i && n) {
                      var y,
                        i = t.concat(t),
                      ...
                      for (var g = c.length, d = m / 2, h = 0; h < g; h += 3) {
                        var f = y[h] + d,
                          b = y[h + 1] + d,
                          _ = y[h + 2] + d;
                        (y[h + g] = _), (y[h + 1 + g] = b), (y[h + 2 + g] = f);
                      }
                      (p.attributes.position.values = i),
                        a &&
                          s.normal &&
                          ((s = p.attributes.normal.values),
                          (p.attributes.normal.values = new Float32Array(
                            i.length
                          )),
                      ...
                      for (
                        m = t.length / 3,
                          y = E.IndexDatatype.createTypedArray(m, c.length),
                          h = 0;
                        h < c.length;
                        h += 3
                      )
                    ...
                    u.topAndBottom = new G.GeometryInstance({ geometry: p });
                  }
                  var p = o.outerRing,
                    v = O.EllipsoidTangentPlane.fromPoints(
                      p,
                      e
                    ).projectPointsOntoPlane(p, N);
                  V.PolygonPipeline.computeWindingOrder2D(v) ===
                    V.WindingOrder.CLOCKWISE && (p = p.slice().reverse());
          ...
          c = A.GeometryPipeline.combineInstances(d)[0];
          (c.attributes.position.values = new Float64Array(
            c.attributes.position.values
          )),
            (c.indices = E.IndexDatatype.createTypedArray(
              c.attributes.position.values.length / 3,
              c.indices
            ));
          (p = c.attributes),
            (a = U.BoundingSphere.fromVertices(p.position.values));
          return (
