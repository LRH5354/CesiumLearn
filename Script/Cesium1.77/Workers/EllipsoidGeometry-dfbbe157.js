define([
...
], function (t, $, e, tt, et, at, it, rt, nt, ot, mt, c) {
  "use strict";
  ...
  function d(t) {
    ...
    var e = $.defaultValue(t.radii, l),
      ...
      u = $.defaultValue(t.vertexFormat, c.VertexFormat.DEFAULT);
  ...
  (d.packedLength =
    2 * et.Cartesian3.packedLength + c.VertexFormat.packedLength + 7),
  ...
  (d.unpack = function (t, e, a) {
    ...
    e += et.Cartesian3.packedLength;
    var n = c.VertexFormat.unpack(t, e, y);
    ...
    return $.defined(a)
      ? ((a._radii = et.Cartesian3.clone(i, a._radii)),
        ...
        (a._vertexFormat = c.VertexFormat.clone(n, a._vertexFormat)),
        (a._minimumClock = o),
        (a._maximumClock = m),
        (a._minimumCone = s),
        (a._maximumCone = u),
      ...
      if (!(e.x <= 0 || e.y <= 0 || e.z <= 0)) {
        ...
        if (!(a.x <= 0 || a.y <= 0 || a.z <= 0)) {
          ...
          var l = 0,
            f = [n],
            c = [i];
          for (B = 0; B < u; B++) f.push(n + (B * (o - n)) / (u - 1));
          for (f.push(o), I = 0; I < s; I++)
            c.push(i + (I * (r - i)) / (s - 1));
          c.push(r);
          var d = f.length,
            C = c.length,
          ...
          var x = C * d * y,
            ...
            D = new Array(d),
            G = new Array(d);
          for (B = 0; B < d; B++) (D[B] = Ct(f[B])), (G[B] = dt(f[B]));
          for (var L = new Array(C), O = new Array(C), I = 0; I < C; I++)
            (O[I] = dt(c[I])), (L[I] = Ct(c[I]));
          for (B = 0; B < d; B++)
            for (I = 0; I < C; I++)
              (b[l++] = e.x * D[B] * O[I]),
                (b[l++] = e.y * D[B] * L[I]),
