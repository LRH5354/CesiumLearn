  ...
  return n(function (a, e) {
    ...
    for (
      var s = O,
        u = (function (a, e, r, n, t) {
          ...
          S.AttributeCompression.zigZagDeltaDecode(s, u, c);
      ...
      for (var k, v = t[y], A = n[y], g = i[y], m = 0; m < v; ++m) {
        0 === m
          ? ((E = L.Cartesian3.unpack(u, 3 * h, Z)),
            (x = L.Cartesian3.unpack(u, 3 * (h + 1), j)),
            (k = L.Cartesian3.subtract(E, x, q)),
            L.Cartesian3.add(E, k, k))
          : (k = L.Cartesian3.unpack(u, 3 * (h + m - 1), q));
        var x,
          E,
          D,
          I = L.Cartesian3.unpack(u, 3 * (h + m), J);
        m === v - 1
          ? ((x = L.Cartesian3.unpack(u, 3 * (h + v - 1), Z)),
            (E = L.Cartesian3.unpack(u, 3 * (h + v - 2), j)),
            (D = L.Cartesian3.subtract(x, E, K)),
            L.Cartesian3.add(x, D, D))
          : (D = L.Cartesian3.unpack(u, 3 * (h + m + 1), K)),
          L.Cartesian3.subtract(k, s, k),
          L.Cartesian3.subtract(I, s, I),
          L.Cartesian3.subtract(D, s, D);
        for (var T = m === v - 1 ? 2 : 4, U = 0 === m ? 2 : 0; U < T; ++U) {
          L.Cartesian3.pack(I, c, d),
    ...
    for (y = 0; y < l; ++y)
      (N[M++] = R),
        ...
        (R += 4);
    return (
      e.push(c.buffer, o.buffer, f.buffer),
