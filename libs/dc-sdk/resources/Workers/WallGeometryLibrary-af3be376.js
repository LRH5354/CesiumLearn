    ...
    if (f.defined(o)) {
      ...
      if (((c.minDistance = g), (c.ellipsoid = e), n)) {
        for (var p = 0, u = 0; u < h - 1; u++)
          p += P.PolylinePipeline.numberOfPoints(i[u], i[u + 1], g) + 1
        ...
        for (u = 0; u < h - 1; u++) {
          ...
          var m = P.PolylinePipeline.generateArc(c)
          l.set(m, y),
            (v[0] = r[u]),
            (v[1] = r[u + 1]),
            s.set(P.PolylinePipeline.generateArc(c), y),
            (y += m.length)
        }
      } else
        (c.positions = i),
          (c.height = t),
          (l = new Float64Array(P.PolylinePipeline.generateArc(c))),
          (c.height = r),
          (s = new Float64Array(P.PolylinePipeline.generateArc(c)))
